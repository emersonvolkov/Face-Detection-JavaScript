module.exports = {
  query: function({ startDate, endDate, startDateOld, endDateOld }) {
    let q = `select
    ed.cod_seccion,
  ...
  from
    ...
    (
      ...
      case
                 when
                    1 = 1
        and fd.fe_fecha between to_date('${startDate}', 'yyyy-mm-dd') and to_date('${endDate}' , 'yyyy-mm-dd') 
                 then
                    'ACTUAL' 
                 else
                    'ANTERIOR' 
    ...
    where
      ...
      and
      (
                 fd.fe_fecha between to_date('${startDate}', 'yyyy-mm-dd') and to_date('${endDate}' , 'yyyy-mm-dd')
      or fd.fe_fecha between to_date('${startDateOld}', 'yyyy-mm-dd')  and to_date('${endDateOld}', 'yyyy-mm-dd')
              )
        )
        fd
    ...
    on ed.cod_seccion = sz.sub_seccion
      ...
      and sz.fecha_corte = to_date('${endDate}', 'yyyy-mm-dd')
    left join
    (
           select
      mz.mzo_seccion,
      ...
      sum( 
              case
                 when
                    lz.fecha_corte = to_date('${endDate}', 'yyyy-mm-dd') 
                 then
                    lz.loc_cuerpo 
                 else
                    0 
              ...
              case
                 when
                    lz.fecha_corte = to_date('${endDateOld}', 'yyyy-mm-dd') 
                 then
                    lz.loc_cuerpo 
                 else
                    0 
    ...
    where
              lz.fecha_corte in 
              (
                 to_date('${endDate}', 'yyyy-mm-dd'), to_date('${endDateOld}', 'yyyy-mm-dd')
              )
      and 1 = 1
